...
function quickSort(array)
{
    ...
    const miniNumber = [];                     //todo      on prépare le tableau de mini pour mettre les petit chiffres 
    ...
    if (miniNumber.length > 0 && maxiNumber.length > 0) { //todo    on dit que si jamais il y a quelque chose dans le tableau des mini et maxi alors on lance l'algo
        ...
        console.log(miniNumber + " / " + maxiNumber); //? on fait un console.log pour avoir un visuelle de l'opération

          return [...quickSort(miniNumber), pivot, ...quickSort(maxiNumber)]; //? on fait deux straight operator pour que tout ce presente dans un seul tableau
    }
        else if (miniNumber.length > 0)             //todo s'il y a quelque chose dans le mini tableau mais pas le maxi on lance cette version 
        {
            return [...quickSort(miniNumber), pivot]; //! on oublie pas le pivot
        }
